...
export class HerokuMiaAgent extends BaseChatModel {
    ...
    async _generate(messages, options, runManager) {
        ...
        toolCallChunks.forEach((chunk) => {
            if (!chunk.id)
                return; // Skip chunks without an id
        ...
        toolCallMap.forEach((assembledTc) => {
            if (assembledTc.id && assembledTc.name && assembledTc.args) {
                try {
                    ...
                    console.warn(`Failed to parse tool call arguments for id ${assembledTc.id}: ${assembledTc.args}`, e);
    ...
    async *_stream(messages, options, runManager) {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                if (response.status >= 400 && response.status < 500) {
                    ...
                    lastError = new HerokuApiError(`Heroku Agent API stream request failed: ${errorData.message || response.statusText}`, response.status, errorData);
                    break;
                }
                lastError = new HerokuApiError(`Heroku Agent API stream request failed with status ${response.status}`, response.status);
            }
            catch (error) {
                lastError = error;
            }
        ...
        if (!successfulResponse || !response || !response.body) {
            if (lastError)
                throw lastError;
            throw new HerokuApiError("Failed to connect or get a streaming body from Heroku Agent API.", response?.status);
        }
                ...
                switch (herokuEventType) {
                    ...
                    case "chat.completion.chunk": // Streaming chat completion chunk (contains delta)
                        if (eventDataJSON.choices && eventDataJSON.choices.length > 0) {
                            ...
                            let content = "";
                            let toolCallChunks = undefined;
                            if (delta.content) {
                                content = delta.content;
                                runManager?.handleLLMNewToken(content);
                            }
                            if (delta.tool_calls && delta.tool_calls.length > 0) {
                                toolCallChunks = delta.tool_calls.map((tc, index) => ({
                                    id: tc.id,
                    ...
                    case "tool.call": // If event name itself is tool.call as per SPECS for agent-initiated tool call phase
                        const toolCallData = eventDataJSON; // Assuming eventDataJSON is the `data` part
                        const toolCallChunk = {
                            id: toolCallData.id,
                    ...
                    case "tool.error": // As per SPECS
                        ...
                        runManager?.handleLLMError(new Error(`Tool '${toolErrorData.name || toolErrorData.id}' failed: ${toolErrorData.error}`));
                        yield new AIMessageChunk({
                            content: "",
                            additional_kwargs: {
                    ...
                    case "agent.error": // As per SPECS
                        ...
                        throw new HerokuApiError(`Agent error: ${agentErrorData.message}`, undefined, agentErrorData);
