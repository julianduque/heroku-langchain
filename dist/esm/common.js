...
export function getHerokuConfigOptions(apiKey, apiUrl, apiEndpoint) {
    ...
    if (!resolvedApiKey) {
        throw new Error("Heroku API key not found. Please set the INFERENCE_KEY environment variable or pass it to the constructor.");
    }
    const resolvedApiUrlBase = apiUrl ?? process.env.INFERENCE_URL ?? DEFAULT_INFERENCE_URL;
