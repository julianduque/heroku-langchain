...
class HerokuMiaAgent extends chat_models_1.BaseChatModel {
    ...
    invocationParams(options) {
        ...
        if (options) {
            ...
            if (options.metadata)
                runtimeParams.metadata = options.metadata;
            if (options.sessionId)
                runtimeParams.sessionId = options.sessionId;
    ...
    async _generate(messages, options, runManager) {
        ...
        let finalAIMessageChunk;
        let finish_reason = null;
        const additional_kwargs = {};
        let tool_calls = [];
        let tool_results = undefined;
        for await (const chunk of stream) {
            if (chunk.content) {
    ...
    async *_stream(messages, options, runManager) {
        ...
        const herokuConfig = (0, common_1.getHerokuConfigOptions)(this.apiKey, this.apiUrl, agentApiEndpoint);
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                if (response.status >= 400 && response.status < 500) {
                    ...
                    lastError = new common_1.HerokuApiError(`Heroku Agent API stream request failed: ${errorData.message || response.statusText}`, response.status, errorData);
                    break;
                }
                lastError = new common_1.HerokuApiError(`Heroku Agent API stream request failed with status ${response.status}`, response.status);
            }
            catch (error) {
                lastError = error;
            }
        ...
        if (!successfulResponse || !response || !response.body) {
            if (lastError)
                throw lastError;
            throw new common_1.HerokuApiError("Failed to connect or get a streaming body from Heroku Agent API.", response?.status);
        }
        try {
            for await (const parsedEvent of (0, common_1.parseHerokuSSE)(response.body)) {
                ...
                try {
                    ...
                    runManager?.handleLLMError(new common_1.HerokuApiError("Invalid JSON in agent stream event", undefined, { event: parsedEvent.event, data: parsedEvent.data }));
                    continue;
                }
                ...
                switch (herokuEventType) {
                    ...
                    case "chat.completion.chunk": // Streaming chat completion chunk
                        if (eventDataJSON.choices && eventDataJSON.choices.length > 0) {
                            ...
                            if (delta.tool_calls && delta.tool_calls.length > 0) {
                                response_metadata.tool_calls = delta.tool_calls.map((tc) => {
                                    ...
                                    if (originalTool) {
                                        ...
                                        if (originalTool.type === "heroku_tool" &&
                                            ...
                                            if (Object.keys(toolCallInfo.args).length === 0) {
                                                toolCallInfo.args = {
                                                    target_app_name: originalTool.runtime_params.target_app_name,
                            ...
                            yield new messages_1.AIMessageChunk({
                                content: content,
                                additional_kwargs: {
                                    finish_reason: choice.finish_reason,
                                    usage: eventDataJSON.usage,
                    ...
                    case "agent.error": // Agent error
                        const agentErrorData = eventDataJSON;
                        runManager?.handleLLMError(new Error(`Agent error: ${agentErrorData.message}`));
                        throw new common_1.HerokuApiError(`Agent error: ${agentErrorData.message}`, undefined, agentErrorData);
                    default:
                        console.warn(`Unknown Heroku Agent event object type: ${herokuEventType}`, eventDataJSON);
                        break;
                }
            }
        }
...
exports.HerokuMiaAgent = HerokuMiaAgent;
//# sourceMappingURL=heroku-mia-agent.js.map