...
export interface HerokuToolMessageContent {
  ...
  // name?: string; // Optional: Heroku docs show name for function in tool_call, not explicitly here
}
...
export interface HerokuChatMessage {
  ...
  content: string | HerokuToolMessageContent[]; // string for system, user, assistant (text); array for tool role
  name?: string; // Optional: Used for function name in tool_calls, or tool_call_id in tool message
  tool_calls?: HerokuToolCall[]; // For assistant message if it makes tool calls
  tool_call_id?: string; // For tool message, linking to assistant's tool_call
}
...
export interface HerokuChatCompletionRequest {
  ...
  tool_choice?:
    ...
    | "required"
    | { type: "function"; function: { name: string } };
  extended_thinking?: boolean; // Example of an additionalKwargs passthrough
  // Potentially other fields from additionalKwargs
  [key: string]: any;
}
...
export interface HerokuChatCompletionChoice {
  index: number;
  message: HerokuChatMessage; // Specifically an assistant message
...
export interface HerokuChatCompletionStreamChoiceDelta {
  role?: HerokuChatMessageRole; // Typically "assistant"
  content?: string | null; // Null if only tool_calls are being streamed
  tool_calls?: Partial<HerokuToolCall>[]; // Incremental tool call chunks
}
...
export interface HerokuChatCompletionStreamChoice {
  ...
  finish_reason: string | null; // Null until the last chunk for that choice
  // logprobs?: any; // if Heroku supports logprobs in stream
}
...
export interface HerokuChatCompletionStreamResponse {
  ...
  // usage?: HerokuChatCompletionUsage; // Usage might appear in the last chunk or not at all
}

// --- LangChain Compatibility Types (if not directly importable/mismatch) ---
...
export interface HerokuAgentCallOptions extends BaseChatModelCallOptions {
  ...
  additionalKwargs?: Record<string, any>; // Add additionalKwargs here
}
...
// Non-streaming response from /invoke (structure TBD, assuming it might be like a final chat message or agent finish state)
export interface HerokuAgentInvokeResponse {
  // Example structure - needs confirmation from actual API spec
  message?: HerokuChatMessage; // Final response message
  tool_results?: any[]; // If invoke can run tools and return final results
  session_id?: string;
  error?: any;
}
...
export interface HerokuAgentMessageDeltaEvent {
  event: "message.delta";
  data: {
    delta: string; // The content delta
    // Any other fields Heroku sends with message.delta
  };
}
...
export interface HerokuAgentToolCallEvent {
  ...
  data: {
    ...
    // Any other fields Heroku sends with tool.call
  };
}
...
export interface HerokuAgentToolCompletionEvent {
  ...
  data: {
    id: string; // Tool call ID (matches a tool.call event)
    name: string; // Tool name
    output: string; // Tool output (JSON string or plain text)
    // Any other fields Heroku sends with tool.completion
  };
}
...
export interface HerokuAgentToolErrorEvent {
  event: "tool.error";
  data: {
    id?: string; // Tool call ID if error is associated with a specific call
    name?: string; // Tool name if applicable
    error: string; // Error message
    // Any other fields Heroku sends with tool.error
  };
}
...
export interface HerokuAgentAgentErrorEvent {
  ...
  data: {
    message: string;
    // Any other fields Heroku sends with agent.error
  };
}
...
export interface HerokuAgentStreamEndEvent {
  event: "stream.end";
  data: {
    final_message?: HerokuChatMessage; // Optional final message from the agent
    // Any other fields Heroku sends with stream.end
  };
}

/** Union type for all possible Heroku Agent SSE data payloads (after JSON parsing `data` field of ParsedSSEEvent) */
export type HerokuAgentSSEData =
  | HerokuAgentMessageDeltaEvent["data"]
  | HerokuAgentToolCallEvent["data"]
  | HerokuAgentToolCompletionEvent["data"]
