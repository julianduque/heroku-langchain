...
export class HerokuMiaEmbeddings extends Embeddings {
  ...
  constructor(fields?: HerokuMiaEmbeddingsFields) {
    ...
    if (!this.model) {
      throw new Error(
        "Heroku embeddings model ID not found. Please set it in the constructor, " +
  ...
  private invocationParams(
    ...
    let runtimeParams: Partial<HerokuEmbeddingsRequest> = {};
    if (options) {
      if (options.model !== undefined) runtimeParams.model = options.model;
  ...
  private async makeRequest(
    requestPayload: HerokuEmbeddingsRequest,
  ): Promise<HerokuEmbeddingsResponse> {
    const herokuConfig = getHerokuConfigOptionsWithEnvKeys(
      this.apiKey,
      this.apiUrl,
      "/v1/embeddings",
      "EMBEDDING_KEY",
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (!response.ok) {
          ...
          const error = new HerokuApiError(
        ...
        const data: HerokuEmbeddingsResponse = await response.json();
        return data;
      } catch (error) {
  ...
  private async _embedDocuments(
    ...
    const requestPayload: HerokuEmbeddingsRequest = {
    ...
    Object.keys(requestPayload).forEach(
      (key) =>
        requestPayload[key as keyof HerokuEmbeddingsRequest] === undefined &&
        delete requestPayload[key as keyof HerokuEmbeddingsRequest],
    );
